...
const TYPE_ICON = (l: Lesson) => l.assignmentId ? '📝' : l.videoUrl ? '▶' : '🧪';
const TYPE_COLOR = (l: Lesson) => l.assignmentId
  ? 'text-orange-600 bg-orange-50'
  : l.videoUrl
  ? 'text-blue-600 bg-blue-50'
  : 'text-green-600 bg-green-50';
const TYPE_LABEL = (l: Lesson) => l.assignmentId ? 'Assignment' : l.videoUrl ? 'Video' : 'Lab';

const NOTES: Record<string, { content: string; code?: string }> = {
  'lesson-0-1': {
    content: 'Scratch is a visual programming language developed by MIT. It uses drag-and-drop blocks to represent code constructs like loops, conditionals, and variables.',
...
export default function LearnPage({ params }: { params: { slug: string } }) {
  ...
  const [activeTab, setActiveTab] = useState<'notes' | 'code' | 'assignment'>('notes');
  ...
  const handleSubmit = async () => {
    ...
    if (!token) { setSubmitResult('⚠️ Please sign in to submit assignments.'); return; }
    if (!activeLesson?.assignmentId) { setSubmitResult('⚠️ No assignment for this lesson.'); return; }
    setSubmitting(true); setSubmitResult('');
    try {
    ...
    } catch { setSubmitResult('❌ Could not reach the server.'); }
  ...
  return (
    <div className="flex h-[calc(100vh-57px)] overflow-hidden bg-white">
      ...
      {/* LEFT SIDEBAR */}
      <aside className="w-72 shrink-0 border-r border-gray-200 flex flex-col overflow-hidden bg-gray-50">
              ...
              {openModules.includes(mod.id) && (mod.lessons || []).map((lesson) => (
                <button key={lesson.id} onClick={() => setActiveId(lesson.id)}
                  ...
                  <span className="mt-0.5 shrink-0 text-sm">
                    {completed.includes(lesson.id) ? <span className="text-green-500">✓</span> : TYPE_ICON(lesson)}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className={`font-medium leading-snug truncate ${activeId === lesson.id ? 'text-[#0077cc]' : 'text-gray-700'}`}>
                      {lesson.title}
                    </p>
                    <p className="text-gray-400 mt-0.5">{lesson.summary?.slice(0, 40)}…</p>
                  </div>
                </button>
              ))}
            </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Video player */}
        <div className="bg-black flex items-center justify-center" style={{ height: '42%' }}>
          <div className="w-full h-full relative flex items-center justify-center">
            {activeLesson?.videoUrl && activeLesson.videoUrl !== 'https://www.example.com/scratch-intro.mp4'
              && !activeLesson.videoUrl.includes('example.com') ? (
              <iframe src={activeLesson.videoUrl} className="w-full h-full" allowFullScreen title={activeLesson.title} />
            ) : (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                <div className="h-16 w-16 rounded-full bg-white/20 flex items-center justify-center cursor-pointer hover:bg-white/30 transition">
                  <svg className="h-8 w-8 text-white ml-1" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" />
                  </svg>
                </div>
                <p className="text-white/70 text-sm">{activeLesson?.title}</p>
                <p className="text-white/40 text-xs">Video will play when a real URL is set</p>
              </div>
            )}
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 px-4 py-3 flex items-center gap-3">
              <div className="flex-1 bg-white/20 rounded-full h-1"><div className="bg-[#0077cc] h-1 rounded-full w-0" /></div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center border-b border-gray-200 bg-white px-6">
            {(['notes', 'code', 'assignment'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition ${
                  activeTab === tab ? 'border-[#0077cc] text-[#0077cc]' : 'border-transparent text-gray-500 hover:text-gray-800'
                }`}>
                {tab === 'notes' ? '📖 Notes' : tab === 'code' ? '💻 Code Editor' : '📝 Assignment'}
              </button>
            ))}
            <div className="ml-auto flex items-center gap-3 py-2">
              {activeLesson && (
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${TYPE_COLOR(activeLesson)}`}>
                  {TYPE_LABEL(activeLesson)}
                </span>
              )}
              ...
              <div className="max-w-3xl space-y-6">
                    ...
                    {note.code && (
                      <div className="rounded-xl bg-[#1e1e1e] overflow-hidden">
                        <div className="flex items-center gap-2 px-4 py-2 bg-[#2d2d2d]">
                          ...
                          <span className="ml-2 text-xs text-gray-400">example.c</span>
                        </div>
                ...
                {activeLesson.assignmentId && (
                  <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">
                    ...
                    <p>This lesson has a graded assignment. Switch to the Assignment tab to submit your solution.</p>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="max-w-3xl space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <select className="text-xs border border-gray-300 rounded px-2 py-1 text-gray-700">
                    <option>C</option><option>Python</option><option>JavaScript</option>
                  </select>
                </div>
                <div className="rounded-xl border border-gray-200 overflow-hidden">
                  <div className="bg-[#1e1e1e] px-4 py-2 flex items-center gap-2">
                    ...
                    <span className="h-3 w-3 rounded-full bg-yellow-500" />
                    <span className="h-3 w-3 rounded-full bg-green-500" />
                    <span className="ml-2 text-xs text-gray-400">main.c</span>
                  </div>
                  <textarea value={code} onChange={e => setCode(e.target.value)}
                    className="w-full bg-[#1e1e1e] text-green-300 font-mono text-sm px-5 py-4 outline-none resize-none"
                    rows={14} spellCheck={false} />
                </div>
                <div className="rounded-xl bg-gray-900 px-4 py-3 text-xs text-gray-400 font-mono min-h-[60px]">
                  <p className="text-gray-500 mb-1">$ output</p>
                  <p className="text-green-400">// Code execution coming soon</p>
                </div>
              </div>
            )}

            {/* ASSIGNMENT TAB */}
            {activeTab === 'assignment' && activeLesson && (
              <div className="max-w-3xl space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-gray-900">{activeLesson.title}</h2>
                  <p className="text-sm text-gray-500 mt-1">
                    {activeLesson.assignmentId
                      ? `Assignment ID: ${activeLesson.assignmentId} · Auto-graded on submission`
                      : 'No graded assignment for this lesson.'}
                  </p>
                </div>
                {activeLesson.assignmentId ? (
                  <div className="rounded-xl border border-gray-200 p-5 space-y-4">
                    <p className="text-sm text-gray-600 leading-relaxed">{activeLesson.summary}</p>
                    <div>
                      <p className="font-semibold text-gray-900 text-sm mb-2">Your solution</p>
                      <textarea value={code} onChange={e => setCode(e.target.value)}
                        className="w-full rounded-lg border border-gray-300 bg-[#1e1e1e] text-green-300 font-mono text-sm px-4 py-3 outline-none focus:border-[#0077cc] resize-none"
                        rows={10} placeholder="// Write your code here..." spellCheck={false} />
                    </div>
                    {submitResult && (
                      <p className={`text-sm rounded-lg px-4 py-2.5 ${submitResult.startsWith('✅') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
                        {submitResult}
                      </p>
                    )}
                    ...
                    <div className="rounded-xl border border-gray-200 p-4">
                      ...
                      <div className="border-t border-gray-200 pt-2 flex justify-between text-sm font-bold mt-1">
                        <span>Total</span><span>100 pts</span>
                      </div>
                    </div>
                  </div>
                ) : (
                  <p className="text-gray-400 text-sm">This lesson is a lecture or lab — no submission required.</p>
                )}
              </div>
            )}
          </div>
